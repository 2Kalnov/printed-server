...
public interface DocumentRepository extends JpaRepository<Document, Integer> {
  ...
  @Query(value = "select top 1 Id, UserId, [Name], Size, PagesCount, [File], ContentType, FilePath from Documents where Documents.Id = :docId", nativeQuery = true)
  Optional<Document> findByIdNative(@Param("docId") int docId);

  @Query(value = "select top 1 Id, UserId, [Name], [File], Size, PagesCount, ContentType, FilePath from Documents where Documents.Id = :docId and UserId = :userId", nativeQuery = true)
  Optional<Document> findByIdAndUserIdNative(@Param("docId") int documentId, @Param("userId") int userId);

  @Query(value = "select Id, UserId, [Name], Size, [File], PagesCount, ContentType, FilePath from Documents where UserId = :userId", nativeQuery = true)
  List<Document> getDocumentsByUserIdNative(@Param("userId") int userId);

  @Query(value = "select Documents.Id, Documents.[Name], Documents.ContentType, Documents.UserId, Documents.PagesCount, Documents.Size, Documents.[File], FilePath from\n" +
